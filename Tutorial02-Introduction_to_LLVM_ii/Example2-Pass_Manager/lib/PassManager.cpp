...
/// An analysis pass that hypothetically gathers some information from the
/// module.
class SomeAnalysis final : public AnalysisInfoMixin<SomeAnalysis> {
public:
  /// The information that is collected by @c SomeAnalysis . In this specific
  /// example we are assuming it to be of type vector.
  struct Result : std::vector<unsigned> {
    /// Verify the validity of the analysis results. This method is implicitly
    /// invoked by the pass manager.
